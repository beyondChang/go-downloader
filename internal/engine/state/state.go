...
// ClearCompletedDownloads removes all completed downloads from the database
func ClearCompletedDownloads() error {
	db := getDBHelper()
	if db == nil {
		return fmt.Errorf("database not initialized")
	}

	_, err := db.Exec("DELETE FROM downloads WHERE status = 'completed'")
	return err
}
...
// ValidateIntegrity checks that paused .downloader files still exist and haven't been tampered with.
...
func ValidateIntegrity() (int, error) {
	...
	removed := 0
	expectedDownloaderPaths := make(map[string]struct{}, len(entries))
	...
	for _, e := range entries {
		...
		expectedDownloaderPaths[e.destPath+types.IncompleteSuffix] = struct{}{}
		candidateDirs[filepath.Dir(e.destPath)] = struct{}{}
	}

	// Also include directories of all known downloads so we can clean orphan .downloader
	// files that no longer have corresponding DB entries.
	// Keep .downloader files for any non-completed entry (e.g. downloading after crash).
	...
	for allRows.Next() {
		...
		if status != "completed" {
			expectedDownloaderPaths[dest+types.IncompleteSuffix] = struct{}{}
		}
	}
	...
	for _, e := range entries {
		...
		downloaderPath := e.destPath + types.IncompleteSuffix

		// Check if .downloader file exists
		_, statErr := os.Stat(downloaderPath)
		if os.IsNotExist(statErr) {
			// File missing - remove orphaned DB entry
			utils.Debug("Integrity: .downloader file missing for %s, removing entry %s", e.destPath, e.id)
		...
		if statErr != nil {
			return removed, fmt.Errorf("failed to stat %s: %w", downloaderPath, statErr)
		}

		// If we have a stored hash, verify it
		if e.fileHash != "" {
			matches, err := compareAgainstStoredFileHash(downloaderPath, e.fileHash)
			if err != nil {
				return removed, fmt.Errorf("failed to verify hash for %s: %w", downloaderPath, err)
			}
			if !matches {
				// File has been tampered with - remove entry and corrupted file
				utils.Debug("Integrity: hash mismatch for %s (expected %s), removing", downloaderPath, e.fileHash)
				if err := retryRemove(downloaderPath); err != nil && !os.IsNotExist(err) {
					return removed, fmt.Errorf("failed to remove tampered file %s: %w", downloaderPath, err)
				}
				if err := removeDownloadAndTasks(e.id); err != nil {
	...
	// Remove orphan .downloader files that no longer have matching paused/queued entries.
	for dir := range candidateDirs {
		...
		for _, f := range files {
			...
			downloaderPath := filepath.Join(dir, name)
			if _, ok := expectedDownloaderPaths[downloaderPath]; ok {
				continue
			}
			if err := retryRemove(downloaderPath); err != nil && !os.IsNotExist(err) {
				return removed, fmt.Errorf("failed to remove orphan file %s: %w", downloaderPath, err)
			}
			utils.Debug("Integrity: removed orphan .downloader file %s", downloaderPath)
		}
	}
